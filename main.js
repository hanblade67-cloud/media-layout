...
var I18N = {
  ...
  fr: {
    ...
    urlMediaDownloadAndReplace: "T\xE9l\xE9charger et remplacer",
    urlMediaDownloading: "T\xE9l\xE9chargement de l'image...",
    urlMediaDownloadError: "Erreur de t\xE9l\xE9chargement: {status}",
    urlMediaFileSavedLinkFailed: "Fichier enregistr\xE9, mais \xE9chec de mise \xE0 jour du lien",
    urlMediaSuccess: "Image enregistr\xE9e et lien mis \xE0 jour",
    urlMediaFileSavedPatternNotFound: "Fichier enregistr\xE9, mais lien introuvable dans le texte",
  ...
  vi: {
    ...
    urlMediaClickToSave: "Nh\u1EA5p \u0111\u1EC3 l\u01B0u v\xE0o vault",
    urlMediaDownloadAndReplace: "T\u1EA3i xu\u1ED1ng v\xE0 thay th\u1EBF",
    urlMediaDownloading: "\u0110ang t\u1EA3i h\xECnh \u1EA3nh...",
    urlMediaDownloadError: "L\u1ED7i t\u1EA3i xu\u1ED1ng: {status}",
    urlMediaFileSavedLinkFailed: "T\u1EC7p \u0111\xE3 l\u01B0u, nh\u01B0ng kh\xF4ng th\u1EC3 c\u1EADp nh\u1EADt li\xEAn k\u1EBFt",
    urlMediaSuccess: "H\xECnh \u1EA3nh \u0111\xE3 l\u01B0u v\xE0 li\xEAn k\u1EBFt \u0111\xE3 c\u1EADp nh\u1EADt",
    urlMediaFileSavedPatternNotFound: "T\u1EC7p \u0111\xE3 l\u01B0u, nh\u01B0ng kh\xF4ng t\xECm th\u1EA5y li\xEAn k\u1EBFt trong v\u0103n b\u1EA3n",
    urlMediaError: "L\u1ED7i: {error}",
    urlMediaUnknownError: "l\u1ED7i kh\xF4ng x\xE1c \u0111\u1ECBnh"
  },
  "zh-CN": {
...
function t(app, key, vars) {
  const locale = app.getLanguage?.() ?? "en";
...
var CloneSettingsModal = class _CloneSettingsModal extends import_obsidian.Modal {
  onSubmit;
  initial;
  ...
  onOpen() {
    ...
    new import_obsidian.Setting(contentEl).setName(t(this.app, "animation")).addButton((btn) => {
      btn.setButtonText(this.initial.animation === "popup" ? t(this.app, "animationPopup") : t(this.app, "animationFade")).onClick(() => {
    ...
    new import_obsidian.Setting(contentEl).setName(t(this.app, "backgroundOpacity")).addSlider((slider) => slider.setLimits(0, 100, 1).setValue(this.initial.bgOpacity * 100).setDynamicTooltip().onChange((value) => {
      this.initial.bgOpacity = value / 100;
    }));
    new import_obsidian.Setting(contentEl).setName(t(this.app, "controlsOpacity")).addSlider((slider) => slider.setLimits(0, 100, 1).setValue(this.initial.controlsOpacity * 100).setDynamicTooltip().onChange((value) => {
      this.initial.controlsOpacity = value / 100;
    }));
    new import_obsidian.Setting(contentEl).setName(t(this.app, "hideDelayLabel")).addSlider((slider) => slider.setLimits(0, 100, 1).setValue(this.initial.hideDelay * 10).setDynamicTooltip().onChange((value) => {
      this.initial.hideDelay = value / 10;
    }));
    const buttons = contentEl.createDiv({ cls: "modal-button-container" });
    new import_obsidian.ButtonComponent(buttons).setButtonText(t(this.app, "ok")).setCta().onClick(() => {
      this.onSubmit(this.initial);
      this.close();
    });
    new import_obsidian.ButtonComponent(buttons).setButtonText(t(this.app, "reset")).onClick(() => {
      this.initial = { animation: "popup", bgOpacity: 1, controlsOpacity: 0.35, hideDelay: 2, showAudioHandles: true };
      this.close();
      const newModal = new _CloneSettingsModal(
        this.app,
        this.initial,
        this.onSubmit
      );
    ...
    new import_obsidian.ButtonComponent(buttons).setButtonText(t(this.app, "cancel")).onClick(() => {
      this.onSubmit(null);
      this.close();
    });
  }
...
var MediaLayoutSettingsModal = class _MediaLayoutSettingsModal extends import_obsidian.Modal {
  plugin;
  ...
  onOpen() {
    ...
    new import_obsidian.Setting(contentEl).setName(t(this.app, "showHandlesOnHoverLabel")).addToggle((toggle) => toggle.setValue(handlesOnHoverValue).onChange((value) => {
      handlesOnHoverValue = value;
    }));
    new import_obsidian.Setting(contentEl).setName(t(this.app, "hideDetachButtonLabel")).addToggle((toggle) => toggle.setValue(hideDetachValue).onChange((value) => {
      hideDetachValue = value;
    }));
    new import_obsidian.Setting(contentEl).setName(t(this.app, "modalAnimationLabel")).addToggle((toggle) => toggle.setValue(modalAnimationValue).onChange((value) => {
      modalAnimationValue = value;
    }));
    new import_obsidian.Setting(contentEl).setName(t(this.app, "cloneNearFile")).addToggle((toggle) => toggle.setValue(cloneNearFileValue).onChange((value) => {
      cloneNearFileValue = value;
    }));
    new import_obsidian.Setting(contentEl).setName(t(this.app, "cloneRestoreDelayLabel")).addSlider((slider) => slider.setLimits(0, 1e3, 10).setValue(cloneRestoreDelayValue).setDynamicTooltip().onChange((value) => {
      cloneRestoreDelayValue = value;
    }));
    new import_obsidian.Setting(contentEl).setName(t(this.app, "noteRestoreDelayLabel")).addSlider((slider) => slider.setLimits(0, 1e3, 10).setValue(noteRestoreDelayValue).setDynamicTooltip().onChange((value) => {
      noteRestoreDelayValue = value;
    }));
    const buttons = contentEl.createDiv({ cls: "modal-button-container" });
    new import_obsidian.ButtonComponent(buttons).setButtonText(t(this.app, "ok")).setCta().onClick(async () => {
    ...
    new import_obsidian.ButtonComponent(buttons).setButtonText(t(this.app, "reset")).onClick(async () => {
      ...
      this.close();
      new _MediaLayoutSettingsModal(this.app, this.plugin).open();
    });
    new import_obsidian.ButtonComponent(buttons).setButtonText(t(this.app, "cancel")).onClick(() => {
      this.close();
    });
  ...
  async onload() {
        ...
        if (backups.length > 0) {
          try {
            ...
            if (backupData.notes && Object.keys(backupData.notes).length > 0) {
              ...
              new import_obsidian.Notice(`\u{1F504} Data recovered from backup: ${latestBackup.name}`);
            }
          } catch (e) {
    ...
    this.registerEvent(
      this.app.workspace.on("editor-menu", (menu, _editor, view) => {
        ...
        const isUrlMedia = !isNote && !isClone && media && media.src && media.src.startsWith("http");
        ...
        if (isUrlMedia) {
          menu.addItem(
            (i) => i.setTitle(t(this.app, "urlMediaDownloadAndReplace")).setIcon("download").onClick(async () => {
              const url = media.src;
...
`;
  ...
  async applyLayout(notePath, layout) {
    ...
    for (const { host, media, kind, key } of hosts) {
      ...
      const hasRotate = st.rotate !== void 0 && st.rotate !== 0;
      let w, h;
      if (st.w !== void 0 && st.h !== void 0) {
        w = st.w;
        h = st.h;
      } else if (hasRotate) {
        w = parseFloat(host.style.width) || host.offsetWidth || media?.clientWidth || 0;
        h = parseFloat(host.style.height) || host.offsetHeight || media?.clientHeight || 0;
      } else {
        w = host.getBoundingClientRect().width || media?.clientWidth || 0;
        h = host.getBoundingClientRect().height || media?.clientHeight || 0;
      }
      if (st.detached) {
        this.ensureParentAnchor(host);
      ...
      const rotateAngle = typeof st.rotate === "number" ? st.rotate : void 0;
  ...
  applyState(host, media, kind, st) {
    ...
    const hasRotate = (isClone || kind === "note") && typeof st.rotate === "number" && st.rotate !== 0;
    let w, h;
    if (st.w !== void 0 && st.h !== void 0) {
      w = st.w;
      h = st.h;
    ...
    if (st.detached || isClone || kind === "note") {
      ...
      host.style.top = "0px";
      host.dataset.msDetached = "1";
      const rotateAngle = (isClone || kind === "note") && typeof st.rotate === "number" ? st.rotate : void 0;
      this.applyTransform(host, st.x || 0, st.y || 0, rotateAngle);
      host.dataset.msLocked = st.locked ? "1" : "0";
    } else {
      host.dataset.msDetached = "0";
  ...
  readAbsRect(host) {
    ...
    if (hasRotate) {
      ...
      let x2 = 0, y2 = 0;
      if (transformMatch) {
        x2 = parseFloat(transformMatch[1]) || 0;
        y2 = parseFloat(transformMatch[2]) || 0;
      }
      ...
      const w2 = parseFloat(host.style.width) || host.offsetWidth;
      const h2 = parseFloat(host.style.height) || host.offsetHeight;
  ...
  applyTransform(host, x, y, rotate) {
    const isInFloatLayer = !!host.closest(".ms-float-layer");
    const isReadingMode = !!host.closest(".markdown-reading-view, .markdown-preview-view");
    let finalX = x;
    let finalY = y;
  ...
  realignClonesIfNeeded(notePath) {
    const leaves = this.app.workspace.getLeavesOfType("markdown").filter((leaf) => {
      ...
      return view instanceof import_obsidian.MarkdownView && view.file?.path === notePath;
    });
  ...
  async tryRestoreSingleClone(notePath, cloneId, state, root) {
    ...
    if (newClonedMedia) {
      ...
      const fullState = { ...state, w, h, link: state.link, zIndex: state.zIndex };
  ...
  collectLayout(notePath) {
    ...
    for (const it of this.enumerateHostsInNote(notePath)) {
      ...
      if (isClone) {
        ...
        const hasRotate = typeof clones[it.key]?.rotate === "number" && clones[it.key].rotate !== 0;
        const stateW = clones[it.key]?.w;
        const stateH = clones[it.key]?.h;
        ...
        layout[it.key] = {
          ...
          rotate: clones[it.key]?.rotate
        };
      } else {
        const originals2 = noteData.originals || {};
        const hasRotateNote = typeof originals2[it.key]?.rotate === "number" && originals2[it.key].rotate !== 0;
        const stateWNote = originals2[it.key]?.w;
        const stateHNote = originals2[it.key]?.h;
        ...
        layout[it.key] = {
          ...
          locked: it.host.dataset.msLocked === "1",
          rotate: originals2[it.key]?.rotate
        };
      }
  ...
  ensureHandles(overlay, isClone = false, isNote = false) {
    ...
    const hT = needRotation ? need("ms-handle ms-handle--t ms-handle--rotate") : null;
    const hB = needRotation ? need("ms-handle ms-handle--b ms-handle--rotate") : null;
    const hL = needRotation ? need("ms-handle ms-handle--l ms-handle--rotate") : null;
    const hR = needRotation ? need("ms-handle ms-handle--r ms-handle--rotate") : null;
  ...
  async convertUrlMediaToLocal(url, notePath) {
    try {
      this.info("Starting URL conversion", { url, notePath });
      new import_obsidian.Notice(t(this.app, "urlMediaDownloading"));
      ...
      if (response.status !== 200) {
        new import_obsidian.Notice(t(this.app, "urlMediaDownloadError", { status: response.status }));
      ...
      if (attachmentFolder) {
        ...
        savePath = (0, import_obsidian.normalizePath)(cleanFolder + "/" + fileName);
      } else {
        const noteDir = notePath.substring(0, notePath.lastIndexOf("/")) || "";
        savePath = (0, import_obsidian.normalizePath)((noteDir ? noteDir + "/" : "") + fileName);
      }
      ...
      if (!view) {
        ...
        new import_obsidian.Notice(t(this.app, "urlMediaFileSavedLinkFailed"));
        return;
      }
      ...
      if (newContent !== content) {
        ...
        new import_obsidian.Notice(t(this.app, "urlMediaSuccess"));
      } else {
        this.warn("Content did not change - pattern not found", { url, pattern: pattern.source });
        new import_obsidian.Notice(t(this.app, "urlMediaFileSavedPatternNotFound"));
      }
    } catch (error) {
      ...
      new import_obsidian.Notice(t(this.app, "urlMediaError", { error: errorMsg || t(this.app, "urlMediaUnknownError") }));
    }
  }
  ...
  createCloneHost(container, cloneHost, key, parentKey, x, y, initialize = true, fileName) {
    ...
    setTimeout(async () => {
      ...
      const hasRotate = typeof existingState?.rotate === "number" && existingState.rotate !== 0;
  ...
  async hardCleanupForSwitch(prevNotePath) {
    ...
    if (prevNotePath) {
      const leaves = this.app.workspace.getLeavesOfType("markdown").filter((leaf) => {
        const view = leaf.view;
        return view instanceof import_obsidian.MarkdownView && view.file?.path === prevNotePath;
      });
      if (leaves.length) {
    ...
    const allHosts = document.querySelectorAll(".ms-host[data-ms-key]");
    ...
    const startMove = (e) => {
      ...
      const onUp = async () => {
        ...
        if (isMovingGroup) {
          ...
          const hasRotate = isClone && typeof existingRotate === "number" || isNote && typeof existingRotate === "number";
          const stateToSave = {
            x: rect.x || 0,
            y: rect.y || 0,
          ...
          if (!hasRotate) {
            ...
            stateToSave.rotate = existingRotate;
          }
          if (kind === "note") {
  ...
  _attachResizeHandlers(host, overlay, handles, notePath, key, kind, media) {
    ...
    ["lt", "rt", "lb", "rb"].forEach((keyH) => {
      ...
      const startDrag = (e) => {
        ...
        const isClone2 = !!host.dataset.msParentKey;
        const isDetachedOriginal = !isClone2 && kind !== "note" && host.dataset.msDetached === "1";
        const cur = isDetachedOriginal ? this.readAbsOrig(host) : this.readAbsRect(host);
        ...
        const from = map[keyH];
        const hasME = this.isMediaExtendedActive(host);
        const baseAR = kind === "image" ? startW / Math.max(1, startH) : kind === "video" ? isClone2 || hasME ? 16 / 9 : media ? this.getAR(kind, media) : 16 / 9 : kind === "audio" ? 9 / 1 : 0;
        const controller = new AbortController();
        const rotateAngle = isClone2 ? this.notes[notePath]?.clones?.[key]?.rotate || 0 : kind === "note" ? this.notes[notePath]?.originals?.[key]?.rotate || 0 : 0;
        ...
        const onMove = (ev) => {
          ...
          const isDetached = host.dataset.msDetached === "1" || isClone2;
        ...
        const onUp = async () => {
          ...
          document.body.classList.remove("ms-noselect");
          const bucket = isClone2 ? this.notes[notePath]?.clones : this.notes[notePath]?.originals;
          const existingRotate = bucket?.[key]?.rotate;
          const hasRotate2 = isClone2 && typeof existingRotate === "number" || isNote && typeof existingRotate === "number";
          const rect = this.readAbsRect(host);
          let finalW = rect.w;
          let finalH = rect.h;
          if (hasRotate2) {
          ...
          const stateToSave = {
            ...
            y: rect.y || 0,
            free: host.dataset.msFree === "1",
            detached: host.dataset.msDetached === "1" || isClone2,
            locked: host.dataset.msLocked === "1"
          };
          if (hasRotate2) {
            stateToSave.rotate = existingRotate;
          ...
          if (isClone2 && (kind === "video" || kind === "audio")) {
            stateToSave.fileName = host.dataset.msFileName;
          }
          await this.saveState(notePath, key, stateToSave);
        };
    ...
    if ((isClone || isNote) && handles.t && handles.b && handles.l && handles.r) {
      const cursorCache = {};
      const getRotationCursor = (effectiveAngle) => {
      ...
      rotationHandles.forEach(({ handle, name }) => {
        ...
        const startRotate = (e) => {
          ...
          const currentTransform = host.style.transform || "";
          const rotateMatch = currentTransform.match(/rotate\(([^)]+)deg\)/);
          const startRotate2 = rotateMatch ? parseFloat(rotateMatch[1]) : 0;
          ...
          let currentRotation = startRotate2;
          let currentCursor = getRotationCursor(baseAngle + startRotate2);
          const controller = new AbortController();
          const onMove = (ev) => {
            ev.preventDefault();
            document.body.style.cursor = currentCursor;
            handle.style.cursor = currentCursor;
            handle.style.setProperty("cursor", currentCursor, "important");
            ...
            let newRotate = startRotate2 + deltaAngle;
            if (ev.shiftKey) {
